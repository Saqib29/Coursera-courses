...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        print()
        print("Category: {}".format(catyegory))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        ...
        if self.difficulty < rand_num:
            return True
        elif self.difficulty >= rand_num:
            return False
    def getPossibleLetters(self, guessed):
        
        lst = []
        for l in LETTERS:
            if (l in LETTERS) and (l not in guessed) and (l not in lst):
                if l in VOWELS and self.prizeMoney >= VOWEL_COST:
